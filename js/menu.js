class MenuManager {
    ...
    showGameOver(score, wave, level, stage) {
        ...
        this.upgradeScreen.classList.add('hidden');
        this.finalScore.textContent = `得分: ${score}`;
        this.finalWave.textContent = `到达波次: ${wave}`;
        this.finalStage.textContent = `到达关卡: ${stage || 1}`;
        this.finalLevel.textContent = `等级: ${level}`;
    }
    ...
    showStageComplete(stage, wave, score) {
        ...
        screen.classList.remove('hidden');
        stats.innerHTML = `第 ${stage} 关 完成!<br>到达波次: ${wave}<br>当前得分: ${score}`;
    }
